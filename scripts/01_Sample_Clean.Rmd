...
## Load data
```{r}
result = read_csv(here('00_Intermediate_output/step.TAC.final.csv'))
```
...
write_csv(result.valid, here('00_Intermediate_output/step.TAC.final[valid].csv'))
```
...
write.csv(dat_V9_final, here('00_Intermediate_output/Cov_dat_V9.csv'))
```
...
# Load and clean data
## Metabolic outcome
dat_cov <- read_csv(here('00_Intermediate_output/Cov_dat_V9.csv')) %>%
  select(-`...1`)

dat_bio <- read_dta(here('WIT_STEP/Met_Outcome.dta')) %>%
  filter(visit == '9') %>%
...
## FAT(%), FAT MASS(Kg)
dat_FAT <- read_dta(here('WIT_STEP/FAT_Outcome.dta'))
...
dat.step <- read.csv(here('00_Intermediate_output/step.TAC.final[valid].csv'))
...
write.csv(dat_vis, here('00_Intermediate_output/09_Step_Cov_WithoutSmokingImpute.csv'))
```
