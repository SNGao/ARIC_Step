...
write_csv(tmp, here('Results/Tables/01_Step.distribution.beforeExtreme.csv'))
```
...
write_csv(dat_vis, here('00_Intermediate_output/03_Step_Cov_Imputed.csv'))
```

## Exclude extreme values
by Adaptive Trimmed Mean Estimator
```{r}
result = read_csv(here('00_Intermediate_output/03_Step_Cov_Imputed.csv'))
...
write_csv(result_no_extreme, 
          here('00_Intermediate_output/03_Step_Cov_Imputed_ExcludeExtreme.csv'))

# Reshape data for step count distribution analysis
result = read.csv(here('00_Intermediate_output/03_Step_Cov_Imputed_ExcludeExtreme.csv'))
result %>%
...
write_csv(tmp, here('Results/Tables/01_Step.distribution.afterExtreme.csv'))
```

## Extract steps for distribution
```{r}
dat.step = read_csv(here('00_Intermediate_output/03_Step_Cov_Imputed.csv')) |>
  select(subjectid, ADEPT, Oak, SDT, Stepcount, Verisense)
```
...
writexl::write_xlsx(result.temp, here('Results/Tables/01_Distribution.xlsx'))
```
...
for (i in 1:length(step_algorithms)) {
  for (j in 1:length(step_algorithms)) {
    ...
    # Upper triangular: Draw the Bland-Altman plot
    if (i < j) {
...
png(here('Results/Figures/03_Combined.subtitle.png'),
...
png(here('Results/Figures/03_Combined.png'),
...
png(here('Results/Figures/03_Combined(Scaled).png'), width = 12, height = 12, units = 'in', res = 300)
print(combined_plot)
dev.off()
```
...
png(here('Results/Figures/04_Continuous_Steps.png'), height = 5, width = 10, units = 'in', res = 300)
print(combined_plot)
dev.off()
```
...
for (covariate in cov_continous) {
  ...
  ## Plots
  output_name = paste0('Results/Figures/05_Age_GroupBy_', covariate, '_LR.png')
  png(here(output_name), height = 4, width = 10, units = 'in', res = 300)
...
writexl::write_xlsx(fit_results, here('Results/Tables/05_GroupBy_Conefficients.xlsx'))
```
...
for (covariate in cov_continous) {
  ...
  ## Plots
  output_name = paste0('Results/Figures/05_Age_GroupBy_Covariates/05_Age_GroupBy_', 
                       covariate, '_LR(scaling).png')
  png(here(output_name), height = 4, width = 10, units = 'in', res = 300)
  a = wrap_plots(plot_list, ncol = 5) + 
    plot_layout(guides = "collect") &
    theme(legend.position = 'bottom',
...
writexl::write_xlsx(fit_results,
                    here('Results/Figures/05_Age_GroupBy_Covariates/05_GroupBy_Conefficients(Scaling).xlsx'))
```