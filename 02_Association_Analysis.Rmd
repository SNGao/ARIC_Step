...
# 01.Cross-sectional  with health statues
## Load data
```{r}
...
```{r}
dat.cog = read_dta('/Users/gsn/Library/CloudStorage/OneDrive-JohnsHopkins/【JHU】Aging-RA/EBA_Cog_PA/Derived_Data/gen_Cog_all_derive.dta')
...
t1_v9_SPPB <- dat.table.1 %>%
  select(Age, Sex, Race, Education, Center, 
         BMI, Smoking, Drinking, # Marriage
         `CES-D`, Hypertension, Diabetes, APOE,
         Overall_Group,`Overall performance`, `Language performance`, 
         `Memory performance`,`Executive function performance`,
         SPPB) %>% 
  tbl_summary(by = "Overall_Group",
              statistic = all_continuous() ~ "{mean}({sd})",
  ...
  save_as_docx(path = "Data_output/Tables&Figures/Table1_v5_SPPB_3.docx")
```
